...
//        System.out.println(scrapePopulation("Alabama"));
//        System.out.println(scrapeFlower("Alabama"));
        System.out.println(scrapeFlag("Alabama"));
    }

    private static String scrapePopulation(String state) throws IOException {
        List<String[]> content = new ArrayList<>();
        try {
            CsvReader csvReader = new CsvReader("/Users/changruidi/Project1/Project1Task2/Project1Task2/Project1Task2/fips.csv",
                    ',', Charset.forName("GBK"));
//            csvReader.readHeaders();
        ...
        for (int i = 0; i < content.size(); i++) {
            if (content.get(i)[0].equals(state)) {
                System.out.println(content.get(i)[0]);
    ...
    private static String scrapeFlower(String state) throws IOException {
        ...
        for (int i = 0; i < newsHeadlines.size(); i++) {
            if (newsHeadlines.get(i).html().equals(name)) {
                System.out.println(newsHeadlines.get(i).html());
                System.out.println(newsHeadlines.get(i).parent().parent().parent().child(0).child(0).
                        child(0).child(0).attr("src"));
                return String.valueOf(newsHeadlines.get(i).parent().parent().parent().child(0).child(0).
                        child(0).child(0).attr("src"));
            }
        }
        return null;
    ...
    private static String scrapeFlag(String state) throws IOException {
        String name = state;
        String prefix = "https://www.states101.com";
        ...
        for (int i = 0; i < newsHeadlines.size(); i++) {
            if (newsHeadlines.get(i).attr("alt").startsWith(name)) {
                return prefix + newsHeadlines.get(i).attr("src");
            }
        }
        return null;
    }
